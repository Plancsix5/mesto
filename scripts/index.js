...
const popupName = document.querySelector('.popup__profile-form-input_type_name');
const popupWorkplace = document.querySelector('.popup__profile-form-input_type_workplace');
...
const clickImage = document.querySelector('element__img');
...
popupElement.addEventListener('submit', function(event){
  ...
  cardItem.querySelector('.element__fullscreen-button').addEventListener('click',  () => openPopupImage(elementLink.value,elementName.value));
  cardItem.querySelector('.element__wastebasket-button').addEventListener("click", deleteElement);
...
initialCards.forEach((item) =>{
  const cardItem = cardElement.cloneNode(true);
  cardItem.querySelector('.element__fullscreen-button').addEventListener('click',  () => openPopupImage(item.link,item.name));
  cardItem.querySelector('.element__wastebasket-button').addEventListener("click", deleteElement);
  const likeButton = cardItem.querySelector('.element__like-button');
  likeActive (likeButton);
